...
// TODO(dingxiaoshuai123) change
Status Redis::StringsPKPatternMatchDel(const std::string& pattern, int32_t* ret) {
  rocksdb::ReadOptions iterator_options;
  const rocksdb::Snapshot* snapshot;
  ScopeSnapshot ss(db_, &snapshot);
  iterator_options.snapshot = snapshot;
...
Status Redis::BitOp(BitOpType op, const std::string& dest_key, const std::vector<std::string>& src_keys,
  ...
  *ret = static_cast<int64_t>(dest_value.size());
  ...
  rocksdb::WriteBatch batch;
  batch.Put(handles_[kMetaCF], base_dest_key.Encode(), strings_value.Encode());
  return db_->Write(default_write_options_, &batch);
}
    ...
    if (parsed_strings_value.IsStale()) {
      *ret = 0;
      ...
      if (value.compare(parsed_strings_value.UserValue()) == 0) {
        *ret = 1;
        rocksdb::WriteBatch batch;
        batch.Delete(handles_[kMetaCF], base_key.Encode());
        return db_->Write(default_write_options_, &batch);
      } else {
        *ret = -1;
      }
...
Status Redis::StringsExpireat(const Slice& key, uint64_t timestamp) {
  std::string value;
  ScopeRecordLock l(lock_mgr_, key);
...
Status Redis::StringsTTL(const Slice& key, uint64_t* timestamp) {
  std::string value;
  ScopeRecordLock l(lock_mgr_, key);
