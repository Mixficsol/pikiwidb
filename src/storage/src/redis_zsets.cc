...
Status Redis::ScanZsetsKeyNum(KeyInfo* key_info) {
  ...
  for (iter->SeekToFirst(); iter->Valid(); iter->Next()) {
    ...
    if (!parsed_zsets_meta_value.IsSameType(Type::zset)) {
      continue;
    }
...
Status Redis::ZsetsPKPatternMatchDel(const std::string& pattern, int32_t* ret) {
  rocksdb::ReadOptions iterator_options;
  const rocksdb::Snapshot* snapshot;
  ScopeSnapshot ss(db_, &snapshot);
  iterator_options.snapshot = snapshot;
  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ...
    *ret = cnt;
    ...
    ZSetsMetaValue zsets_meta_value(Type::zset, Slice(str, 4));
...
Status Redis::ZRangebyscore(const Slice& key, double min, double max, bool left_close, bool right_close, int64_t count,
...
Status Redis::ZUnionstore(const Slice& destination, const std::vector<std::string>& keys,
  ...
  *ret = 0;
  ...
  if (s.ok()) {
    ...
    char str[4];
    EncodeFixed32(str, member_score_map.size());
    ZSetsMetaValue zsets_meta_value(Type::zset, Slice(str, 4));
    version = zsets_meta_value.UpdateVersion();
...
Status Redis::ZInterstore(const Slice& destination, const std::vector<std::string>& keys,
  ...
  *ret = 0;
  ...
  if (!have_invalid_zsets) {
    ZSetsScoreKey zsets_score_key(valid_zsets[0].key, valid_zsets[0].version, std::numeric_limits<double>::lowest(),
                                  Slice());
  ...
  if (s.ok()) {
    ...
    char str[4];
    EncodeFixed32(str, final_score_members.size());
    ZSetsMetaValue zsets_meta_value(Type::zset, Slice(str, 4));
    version = zsets_meta_value.UpdateVersion();
    batch.Put(handles_[kMetaCF], base_destination.Encode(), zsets_meta_value.Encode());
  }
