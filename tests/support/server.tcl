...
proc start_server {options {code undefined}} {
    ...
    foreach line $data {
        ...
        set directive [lindex $parts 0]

        if {[llength $parts] > 1} {
            set arguments [lrange $parts 1 end]

            set formatted_args {}
            foreach arg $arguments {
                lappend formatted_args [string trim $arg "{}"]
            }

            dict set config $directive $formatted_args
        } else {
            dict set config $directive ""
        }
    }
    ...
    dict set config log-path ./logs$::port/
